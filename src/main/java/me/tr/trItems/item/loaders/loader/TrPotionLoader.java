...
public class TrPotionLoader extends TrBaseLoader {

    public TrPotionItem load(Plugin plugin, Section itemSection) {
        ...
        return new TrPotionItem(baseItem, potionType, customEffects, color, customName);
    }
    ...
    public List<PotionEffect> getCustomEffects(Section itemSection) {
        ...
        for (String effect : effectsStr) {
            ...
            if (effectSplit.length < 3) {
                main.getLogger().error("Effect " + effect + " not contains enough information.");
                continue;
            }
            NamespacedKey potionEffectTypeKey = ItemManager.getInstance().parseNamespacedKey(effectSplit[0]);
            PotionEffectType potionEffectType = Registry.POTION_EFFECT_TYPE.get(potionEffectTypeKey);
            if (potionEffectType == null) {
                main.getLogger().error("Unknown potion effect type " + potionEffectTypeKey);
                continue;
            }
            try {
                ...
                main.getLogger().error("Duration & Amplifier must be numbers " + effectSplit[1] + " & " + effectSplit[2]);
            }
        }
        return effects;
    }
