...
public class TrBaseLoader {
    protected TrItems main = TrItems.getInstance();


    public TrBaseItem load(Plugin plugin, Section itemSection) {
        ...
        List<TrNBT<?>> nbts = getNBT(plugin, itemSection);
        List<TrEnchants> enchants = getEnchants(itemSection);
        List<TrAttributes> attributes = getAttributes(plugin, itemSection);
        ...
        return new TrBaseItem(ID, name, lore, nbts, enchants, attributes, stack, maxAmount, durability, customModelData, glint, fireResistance, unbreakable, hideTooltip, rarity, version, interactable, file, plugin);
    }
    ...
    public List<TrNBT<?>> getNBT(Plugin plugin, Section item) {
        final List<TrNBT<?>> result = new ArrayList<>();
        ...
        for (String specifiedNBT : specifiedNBTs) {
            ...
            if (splitNBT.length < 2) {
                main.getLogger().error("NBT " + specifiedNBT + " not contains enough information.");
                continue;
            }
            ...
            result.add(new TrNBT<>(compound, key, value));
        }
        return result;
    }


    public List<TrEnchants> getEnchants(Section item) {
        final List<TrEnchants> result = new ArrayList<>();
        ...
        for (String specifiedEnchant : specifiedEnchants) {
            ...
            NamespacedKey key = ItemManager.getInstance().parseNamespacedKey(splitEnchant[0]);
            ...
            try {
                ...
                result.add(new TrEnchants(enchantment, level));
    ...
    public List<TrAttributes> getAttributes(Plugin plugin, Section item) {
        final List<TrAttributes> result = new ArrayList<>();
        ...
        for (String specifiedAttribute : specifiedAttributes) {
            ...
            try {
                ...
                result.add(new TrAttributes(attribute, attributeModifier));
            } catch (NumberFormatException e) {
